...
export const initResize: ResizeHandler = (e, setWidth, MIN_WIDTH, MAX_WIDTH) => {
  ...
  const startWidth = parseInt(
    (document.getElementById("chatPanel")?.offsetWidth ?? 300).toString(),
    10
  );
  ...
  function onMouseUp() {
    ...
    document.removeEventListener("touchmove", onMouseMove as (ev: TouchEvent) => void);
  ...
  document.addEventListener("touchmove", onMouseMove as (ev: TouchEvent) => void);
